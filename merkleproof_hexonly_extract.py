...
# --- Main extractor ---
def get_beef(address: str, include_merkle=True, include_txhex=True):
    ...
    for utxo in utxos:
        ...
        # Optional merkle proof and block header
        ...
        # Optional raw transaction hex
        if include_txhex:
            try:
                tx_resp = electrum_request(
                    "blockchain.transaction.get",
                    [utxo_entry["txid"], False]  # False = return raw hex string only
                )
                utxo_entry["tx_hex"] = tx_resp.get("result")
            except Exception:
                utxo_entry["tx_hex"] = None

        beef_utxos.append(utxo_entry)

    return {
        "format": "BEEF",
        "version": 2,  # bumped for tx_hex support
        "address": address,
        "utxos": beef_utxos
    }
